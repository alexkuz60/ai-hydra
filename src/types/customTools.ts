...
{{html}}`,
   ...
   {
     id: 'builtin_evaluate_response',
     name: 'evaluate_response',
     display_name: 'Оценить ответ',
     description: 'Оценивает ответ по критериям фактологичности, релевантности, полноты, стоимости (cost-efficiency) и другим. Используется арбитром для структурированного анализа.',
     parameters: [
       { name: 'response_text', type: 'string', description: 'Ответ для оценки', required: true },
       { name: 'criteria', type: 'string', description: 'Критерии оценки через запятую (например: factuality, relevance, completeness, cost_efficiency). Если пусто, используются все стандартные критерии.', required: false },
       { name: 'context', type: 'string', description: 'Контекст вопроса или задачи для оценки релевантности и полноты', required: false },
       { name: 'token_estimate', type: 'number', description: 'Ориентировочное количество токенов в ответе (для оценки cost-efficiency)', required: false },
     ],
     tool_type: 'prompt',
     is_system: true,
     category: 'ai',
     prompt_template: `Ты — арбитр и оценщик (LLM-as-judge). Проведи структурированную оценку ответа.

**Критерии оценки (0-10 для каждого):**
- **Фактологичность**: Проверь точность фактов, отсутствие ложных утверждений
- **Релевантность**: Насколько прямо ответ относится к вопросу/задаче
- **Полнота**: Охватены ли все важные аспекты? Ответ исчерпывающий?
- **Cost-Efficiency**: Оптимальное соотношение качества к длине ({{token_estimate}} токенов)
- **Ясность**: Хорошо ли структурирован и понятен ответ?
- **Консистентность**: Нет ли внутренних противоречий и логических ошибок?

{{#criteria}}
**Дополнительные критерии**: {{criteria}}
{{/criteria}}
...
|----------|--------|-------------|
| Фактологичность | X/10 | [краткое обоснование] |
| Релевантность | X/10 | [краткое обоснование] |
| Полнота | X/10 | [краткое обоснование] |
| Cost-Efficiency | X/10 | [краткое обоснование] |
| Ясность | X/10 | [краткое обоснование] |
| Консистентность | X/10 | [краткое обоснование] |
...
- [1-2 основных недостатка, если есть]

## Рекомендация
[Использовать / Отклонить / Использовать с уточнением] — [кратко почему]

Будь объективен и справедлив. Обоснуй каждую оценку конкретными примерами из текста.`,
   },
 ];
