...
function renderProviderHint(p: Record<string, unknown>, isRu: boolean, t: (k: string) => string) {
...
export function ApiKeysTab({ apiKeys, keyMetadata, saving, language, t, onKeyChange, onExpirationChange, onSave }: ApiKeysTabProps) {
  ...
  const pi = (key: string) => getProfileText(key, isRu);
  ...
  return (
    <HydraCard variant="glass" className="p-6">
      ...
      <HydraCardContent className="space-y-4">
        <p className="text-sm text-muted-foreground mb-4">{pi('byokDescription')}</p>
        ...
        {LLM_PROVIDERS.map(p => (
          <React.Fragment key={p.provider}>
            <ApiKeyField
              ...
              hint={renderProviderHint(p, isRu, t)}
        ...
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="text-lg font-semibold">{pi('tools')}</h3>
        </div>
