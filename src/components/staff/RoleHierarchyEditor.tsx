    ...
    return (
      <label
        ...
        className={cn(
          ...
          "hover:bg-muted/50",
          isInList && "bg-primary/10 border border-primary/30"
        )}
      >
        <Checkbox
          checked={isInList}
          onCheckedChange={() => toggleRoleInList(role, type)}
          className={cn("h-4 w-4", isInList && "border-primary data-[state=checked]:bg-primary")}
        />
        <div className={cn(
          ...
          <IconComponent className={cn("h-3.5 w-3.5", isInList ? "text-primary" : config.color)} />
        </div>
