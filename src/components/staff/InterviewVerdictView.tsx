...
export function VerdictSection({ session, verdict, phases, isRu, onDecision }: VerdictSectionProps) {
  ...
  const decisionLabels: Record<string, { ru: string; en: string }> = {
    hire: { ru: s('hire', true), en: s('hire', false) },
    reject: { ru: s('reject', true), en: s('reject', false) },
    retest: { ru: s('retestVerb', true), en: s('retestVerb', false) },
  };
    ...
    <div className="space-y-3">
      ...
      <div className="p-3 rounded-md border border-border bg-muted/10">
        <div className="flex items-center gap-2 mb-1">
          ...
          <span className="text-xs font-medium">{s('autoDecision', isRu)}</span>
        ...
        {verdict.thresholds && (
          <div className="flex gap-3 mt-1 text-[10px] text-muted-foreground flex-wrap">
            <span>📊 {s('score', isRu)}: {avgScore}/10</span>
            {verdict.thresholds.current_holder && (
              <span>👤 {s('current', isRu)}: {verdict.thresholds.current_holder.score}</span>
            )}
            {verdict.thresholds.previous_avg !== null && (
              <span>📈 {s('prevAvg', isRu)}: {verdict.thresholds.previous_avg.toFixed(1)}</span>
            )}
          </div>
        )}
      </div>
      ...
      <div className="space-y-1">
        <div className="text-xs font-medium">{s('arbiterScores', isRu)}</div>
      ...
      {!verdict.final_decision && (
        <div className="space-y-2 pt-1">
          <div className="text-xs font-medium">{s('yourDecision', isRu)}</div>
          {isPodium ? (
      ...
      {verdict.final_decision && (
        <div className="p-2 rounded-md border border-border bg-muted/10 flex items-center gap-2">
          ...
          <span className="text-xs font-medium">
            {s('decision', isRu)}: {decisionLabels[verdict.final_decision]?.[isRu ? 'ru' : 'en']}
          </span>
          {verdict.decided_at && (
