      ...
      <div className={cn(
        ...
        <div className="flex items-center gap-2">
          <div className="p-1.5 rounded bg-hydra-archivist/20">
            <HardDrive className="h-4 w-4 text-hydra-archivist" />
          </div>
          <div className="flex-1 min-w-0">
            <div className="text-sm font-medium truncate">{data.label || 'Хранилище'}</div>
            {data.storagePath && (
              <div className="text-xs text-muted-foreground truncate">
                {data.storageOperation}: {data.storagePath}
              </div>
            )}
          </div>
        </div>
