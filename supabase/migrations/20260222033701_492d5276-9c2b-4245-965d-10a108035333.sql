...
-- Add dotpoint_vault_id column if missing
ALTER TABLE public.user_api_keys ADD COLUMN IF NOT EXISTS dotpoint_vault_id uuid;

-- Recreate save_api_key with dotpoint support
...
BEGIN
  v_vault_col := CASE p_provider
    ...
    WHEN 'dotpoint' THEN 'dotpoint_vault_id'
    ELSE NULL
  END;
