-- Добавляем колонки для Tavily и Perplexity
ALTER TABLE public.user_api_keys 
ADD COLUMN IF NOT EXISTS tavily_vault_id UUID,
ADD COLUMN IF NOT EXISTS perplexity_vault_id UUID;

-- Удаляем старые функции и создаём новые с расширенным возвратом
DROP FUNCTION IF EXISTS public.get_my_api_key_status();
DROP FUNCTION IF EXISTS public.get_my_api_keys();

-- Создаём функцию get_my_api_key_status с новыми полями
CREATE FUNCTION public.get_my_api_key_status()
 RETURNS TABLE(has_openai boolean, has_gemini boolean, has_anthropic boolean, has_xai boolean, has_openrouter boolean, has_groq boolean, has_tavily boolean, has_perplexity boolean)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
-- Создаём функцию get_my_api_keys с новыми полями
CREATE FUNCTION public.get_my_api_keys()
 RETURNS TABLE(openai_api_key text, google_gemini_api_key text, anthropic_api_key text, xai_api_key text, openrouter_api_key text, groq_api_key text, tavily_api_key text, perplexity_api_key text)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
BEGIN
  SELECT openai_vault_id, gemini_vault_id, anthropic_vault_id, xai_vault_id, openrouter_vault_id, groq_vault_id, tavily_vault_id, perplexity_vault_id
  INTO v_openai_id, v_gemini_id, v_anthropic_id, v_xai_id, v_openrouter_id, v_groq_id, v_tavily_id, v_perplexity_id
...
-- Обновляем функцию save_api_key для поддержки новых провайдеров
