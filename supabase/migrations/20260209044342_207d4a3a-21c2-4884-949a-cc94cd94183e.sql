...
CREATE FUNCTION public.save_api_key(p_provider text, p_api_key text)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF v_old_vault_id IS NOT NULL THEN
    DELETE FROM vault.secrets WHERE id = v_old_vault_id::uuid;
  END IF;

  IF p_api_key IS NOT NULL AND p_api_key <> '' THEN
    INSERT INTO vault.secrets (secret, name)
    VALUES (p_api_key, p_provider || '_key_' || v_user_id)
    ...
    USING v_new_vault_id::text, v_user_id;
