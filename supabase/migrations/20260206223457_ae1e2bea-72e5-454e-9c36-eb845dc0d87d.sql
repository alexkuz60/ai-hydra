...
-- Add Firecrawl vault column to user_api_keys
ALTER TABLE public.user_api_keys 
ADD COLUMN IF NOT EXISTS firecrawl_vault_id uuid;
...
-- Recreate get_my_api_key_status with Firecrawl
CREATE FUNCTION public.get_my_api_key_status()
 RETURNS TABLE(has_openai boolean, has_gemini boolean, has_anthropic boolean, has_xai boolean, has_openrouter boolean, has_groq boolean, has_tavily boolean, has_perplexity boolean, has_deepseek boolean, has_firecrawl boolean)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
-- Recreate get_my_api_keys with Firecrawl
CREATE FUNCTION public.get_my_api_keys()
 RETURNS TABLE(openai_api_key text, google_gemini_api_key text, anthropic_api_key text, xai_api_key text, openrouter_api_key text, groq_api_key text, tavily_api_key text, perplexity_api_key text, deepseek_api_key text, firecrawl_api_key text)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
DECLARE
  ...
  v_firecrawl_id UUID;
BEGIN
  SELECT openai_vault_id, gemini_vault_id, anthropic_vault_id, xai_vault_id, openrouter_vault_id, groq_vault_id, tavily_vault_id, perplexity_vault_id, deepseek_vault_id, firecrawl_vault_id
  INTO v_openai_id, v_gemini_id, v_anthropic_id, v_xai_id, v_openrouter_id, v_groq_id, v_tavily_id, v_perplexity_id, v_deepseek_id, v_firecrawl_id
...
-- Update save_api_key to handle Firecrawl
...
BEGIN
  ...
  ELSIF p_provider = 'firecrawl' THEN
    SELECT firecrawl_vault_id INTO v_old_secret_id FROM public.user_api_keys WHERE user_id = v_user_id;
  ELSE
  ...
  IF p_api_key IS NULL OR p_api_key = '' THEN
    ...
    ELSIF p_provider = 'firecrawl' THEN
      UPDATE public.user_api_keys SET firecrawl_vault_id = NULL, updated_at = now() WHERE user_id = v_user_id;
    END IF;
    RETURN;
  END IF;
  ...
  ELSIF p_provider = 'firecrawl' THEN
    UPDATE public.user_api_keys SET firecrawl_vault_id = v_secret_id::uuid, updated_at = now() WHERE user_id = v_user_id;
  END IF;
END;
$function$;
