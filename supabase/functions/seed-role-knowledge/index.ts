  ...
  try {
    ...
    if (force) {
      const { error: deleteError } = await supabase
        ...
        .eq('role', role)
        .in('category', ['documentation', 'standard', 'procedure', 'system_prompt', 'rejection_examples'])
        .in('visibility_level', ['role_specific', 'global', 'organizational']);
    ...
    // 2. Seed Level A — Global Mission (shared by all roles)
    const LEVEL_A_CONTENT = `# Стратегическая миссия AI-Hydra

AI-Hydra — это мультиагентная система коллективного интеллекта, основанная на шести столпах:

1. **Ролевые контракты** — каждая роль имеет чёткие должностные инструкции, компетенции и ограничения. Роль не выходит за рамки своего мандата.
2. **Четырёхуровневая верификация (Tier 4)** — каждое решение проходит через генерацию (Эксперт), критику (Критик), арбитраж (Арбитр) и модерацию (Модератор).
3. **Сигналы доверия** — статистика моделей, оценки арбитров, brain-рейтинги формируют объективную картину надёжности каждой модели.
4. **Когнитивное разнообразие** — разные модели с разными архитектурами конкурируют за должности через конкурсы и собеседования.
5. **Самокорректирующаяся эволюция** — система непрерывно оптимизирует свои промпты, параметры и поведение через эволюционные циклы.
6. **Антропоморфная организация** — штат, должности, найм, аттестация, увольнение — метафора корпоративной структуры для управления ИИ-агентами.

Формула интегральной надёжности: V_total = V_competitive × V_adversarial × V_QC × V_semantic.
Цель системы — минимизировать галлюцинации и максимизировать качество через коллективную верификацию.`;

    entries.push({
      user_id: user.id,
      role,
      content: LEVEL_A_CONTENT,
      content_en: null,
      source_title: "Миссия AI-Hydra",
      source_title_en: "AI-Hydra Mission",
      category: "documentation",
      chunk_index: 0,
      chunk_total: 1,
      tags: ["миссия", "стратегия", "философия", "level-a"],
      metadata: { seeded: true, source: "system", visibility_level_tag: "global" },
      visibility_level: "global",
    });

    // 3. Seed Level B — Organizational Staff Map (shared by all roles)
    const LEVEL_B_CONTENT = `# Карта штата AI-Hydra

Ты работаешь в команде из специализированных ролей. Вот краткий обзор коллег и их функций:

## Экспертный отдел
- **Эксперт (assistant)** — универсальный специалист широкого профиля. Генерирует первичные решения и анализирует задачи.
- **Критик (critic)** — выявляет логические изъяны, противоречия и риски в ответах других ролей.
- **Арбитр (arbiter)** — синтезирует финальное решение, взвешивая все точки зрения участников дискуссии.
- **Консультант (consultant)** — отвечает на прямые вопросы пользователя в режиме быстрой консультации.
- **Модератор (moderator)** — обобщает дискуссию, выделяя консенсус и ключевые расхождения.
- **Советник (advisor)** — предлагает стратегические рекомендации и планы действий.

## Технический персонал
- **Архивариус (archivist)** — управляет памятью системы: сохраняет, ищет и структурирует знания сессий и ролей.
- **Аналитик (analyst)** — анализирует статистику моделей, эффективность и поведенческие паттерны.
- **Веб-Охотник (webhunter)** — выполняет поиск информации в интернете через поисковых провайдеров.
- **Промпт-Инженер (promptengineer)** — оптимизирует системные промпты и управляет библиотекой промптов.
- **Регулятор потоков (flowregulator)** — проектирует и валидирует flow-диаграммы автоматизированных процессов.
- **Кузнец инструментов (toolsmith)** — создаёт и настраивает пользовательские инструменты (prompt/HTTP).
- **Экскурсовод (guide)** — обучает пользователей навигации по платформе и объясняет функционал.

## Отдел ТехКонтроля (ОТК) — системные роли
- **ТехноКритик (technocritic)** — автоматизированный судья для критического анализа в конкурсах.
- **ТехноАрбитр (technoarbiter)** — автоматизированный судья для вынесения вердиктов в конкурсах.
- **ТехноМодератор (technomoderator)** — автоматизированный резюмист для обобщения результатов.
- **Переводчик (translator)** — выполняет перевод контента между языками с сохранением терминологии.
- **Эволюционер (evolutioner)** — оптимизирует промпты через эволюционные циклы.
- **Хроникёр (chronicler)** — ведёт летопись изменений системы.

## Юридический отдел
- **Патентный поверенный (patent_attorney)** — анализирует правовые аспекты и интеллектуальную собственность.

Взаимодействуй с коллегами строго в рамках своей компетенции. Не подменяй функции других ролей.`;

    entries.push({
      user_id: user.id,
      role,
      content: LEVEL_B_CONTENT,
      content_en: null,
      source_title: "Карта штата",
      source_title_en: "Staff Map",
      category: "documentation",
      chunk_index: 0,
      chunk_total: 1,
      tags: ["штат", "роли", "организация", "level-b"],
      metadata: { seeded: true, source: "system", visibility_level_tag: "organizational" },
      visibility_level: "organizational",
    });

    // 4. Seed role-specific knowledge (Level C)
