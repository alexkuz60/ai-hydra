...
import { PROXYAPI_TO_LOVABLE_MAP, resolveProxyApiModel } from "../_shared/proxyapi.ts";
...
async function callPersonalModel(
  ...
  if (provider === "proxyapi") {
    const realModel = resolveProxyApiModel(model);
    ...
    if (!response.ok) {
      ...
      if (response.status === 404) {
        const lovableModelId = PROXYAPI_TO_LOVABLE_MAP[model];
        if (lovableModelId) {
